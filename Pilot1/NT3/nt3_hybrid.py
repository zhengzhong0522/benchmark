...
# use Horovod to parallelize model.fit
import tensorflow as tf
...
import time
from ctypes import *
#powerlib = CDLL("/home/wuxf/Theta/maide/PoLiMEr/lib/libpolimer.so")
...
#model.add(Activation('softmax'))
    ...
    callbacks = [hvd.callbacks.BroadcastGlobalVariablesCallback(0), csv_logger, reduce_lr, candleRemoteMonitor, timeoutMonitor]

# Horovod: save checkpoints only on worker 0 to prevent other workers from corrupting them.
    #if hvd.rank() == 0:
        #callbacks.append(ModelCheckpoint('/tmp/checkpoint-{epoch}.h5'))
...
# Horovod: Evaluate the model on the full data set.
    #score = hvd.allreduce(model.evaluate(X_test, Y_test, verbose=0),average=False)

#Finalize the power lib
    print ("Runtime: ", time.time()-t0, "\n")
#    powerlib.poli_finalize()
