...
def load_X_data(train_file, test_file,
    ...
    df_train = pd.read_csv(train_file, engine='c', usecols=usecols, low_memory=False)
    df_test = pd.read_csv(test_file, engine='c', usecols=usecols, low_memory=False)
...
def load_Xy_data_noheader(train_file, test_file, classes, usecols=None, scaling=None, dtype=DEFAULT_DATATYPE):
    ...
    """
    print('Loading data...')
    df_train = (pd.read_csv(train_file, header=None, usecols=usecols, low_memory=False).values).astype(dtype)
    df_test = (pd.read_csv(test_file, header=None, usecols=usecols, low_memory=False).values).astype(dtype)
    print('done')
...
def load_csv_data(train_path, test_path=None, sep=',', nrows=None,
    ...
    if x_cols is None and drop_cols is None and n_cols is None:
        ...
        df_cols = pd.read_csv(train_path, engine='c', sep=sep, nrows=0, low_memory=False)
    ...
    df_train = pd.read_csv(train_path, engine='c', sep=sep, nrows=nrows, usecols=usecols, low_memory=False)
    if test_path:
        df_test = pd.read_csv(test_path, engine='c', sep=sep, nrows=nrows, usecols=usecols, low_memory=False)
    else:
